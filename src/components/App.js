...
class App extends React.Component {
  ...
  componentDidMount() {
    ...
    zt.bind(rotatingElement, "rotate", (event) => {
      ...
      this.length = this.state.main_menu_active? this.state.main_menu_list.length: this.state.music_menu_list.length;
  ...
  prevButtonClickEvent = () =>{
   ...
   playPauseClickEvent = () => {
    if(this.state.music_menu_active && this.state.music_menu_current === 0){
        if($(".display-items .width-50").length === 0){
            if( $('.music-player')[0].paused !== undefined && $('.music-player')[0].paused){
                console.log('hi');
                $('.music-player')[0].play();
                this.setState({
